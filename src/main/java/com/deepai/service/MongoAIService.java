...
public class MongoAIService {
    ...
    public List<Document> vectorSearch(String dbName, String collectionName, String vectorFieldName, 
        ...
        try {
            // Parse vector array
            Document vectorDoc = Document.parse(vectorArrayJson);
            @SuppressWarnings("unchecked")
            List<Double> vector = (List<Double>) vectorDoc.get("vector");
            
            if (vector == null || vector.isEmpty()) {
                return List.of(new Document("error", "Invalid vector provided"));
            }
            
            // Build vector search aggregation pipeline
            List<Document> pipeline = new ArrayList<>();
            
            // $vectorSearch stage (MongoDB Atlas Vector Search)
            Document vectorSearchStage = new Document("$vectorSearch", 
                new Document("index", vectorFieldName + "_vector_index")
            ...
            pipeline.add(new Document("$addFields", 
                new Document("similarityScore", 
                    new Document("$meta", "vectorSearchScore"))));
            
            // Execute aggregation
            List<Document> results = mongoServiceClient.getMongoClient()
                    .getDatabase(dbName)
    ...
    public Map<String, Object> aiAnalyzeDocument(String dbName, String collectionName, String documentId, 
        ...
        try {
            ...
            Document document = mongoServiceClient.getMongoClient()
                    .getDatabase(dbName)
                    .getCollection(collectionName)
                    .find(query)
                    .first();
            ...
            if (document == null) {
                ...
                error.put("error", "Document not found with ID: " + documentId);
                return error;
            }
            ...
            logger.error("Failed to analyze document {} in {}.{}: {}", documentId, dbName, collectionName, e.getMessage());
            Map<String, Object> error = new HashMap<>();
    ...
    public Map<String, Object> aiAnalyzeCollection(String dbName, String collectionName, String analysisType, int sampleSize) {
        logger.info("Analyzing collection {}.{} with AI", dbName, collectionName);
        try {
            // Get sample documents
            List<Document> sampleDocs = mongoServiceClient.getMongoClient()
                    .getDatabase(dbName)
                    .getCollection(collectionName)
                    .find()
                    .limit(sampleSize > 0 ? sampleSize : 100)
                    .into(new ArrayList<>());
    ...
    public Map<String, Object> aiQuerySuggestion(String dbName, String collectionName, String queryIntent, String userQuery) {
        ...
        try {
            ...
            suggestions.put("timestamp", new Date());
            
            // Analyze existing indexes for optimization
            List<Document> indexes = mongoServiceClient.listIndexes(dbName, collectionName);
    ...
    public Map<String, Object> aiDocumentSummary(String dbName, String collectionName, String documentId, String summaryType, int maxLength) {
        ...
        try {
            ...
            logger.error("Failed to generate summary for document {} in {}.{}: {}", documentId, dbName, collectionName, e.getMessage());
            Map<String, Object> error = new HashMap<>();
            error.put("error", e.getMessage());
            return error;
        }
    ...
    public List<Document> semanticSearch(String dbName, String collectionName, String naturalLanguageQuery, 
        ...
        try {
            ...
            if (!keywords.isEmpty()) {
                try {
                    ...
                    mongoServiceClient.getMongoClient()
                            .getDatabase(dbName)
                            .getCollection(collectionName)
                            .find(textQuery)
    ...
    public Map<String, Object> generateEmbeddings(String dbName, String collectionName, String textField, 
        ...
        try {
            ...
            List<Document> documents = mongoServiceClient.getMongoClient()
                    .getDatabase(dbName)
                    .getCollection(collectionName)
                    .find(query)
                    .limit(batchSize > 0 ? batchSize : 100)
                    .into(new ArrayList<>());
            ...
            for (Document doc : documents) {
                try {
                    ...
                    logger.warn("Failed to generate embedding for document {}: {}", doc.get("_id"), docError.getMessage());
                    processed++;
                }
            }
    ...
    private List<String> extractSearchKeywords(String naturalLanguageQuery) {
        ...
        String[] words = naturalLanguageQuery.toLowerCase()
                .replaceAll("[^a-zA-Z0-9\\s]", "")
                .split("\\s+");
        
        List<String> keywords = new ArrayList<>();
        Set<String> stopWords = Set.of("the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by", "is", "are", "was", "were", "find", "search", "get", "show", "list");
    ...
    private List<Document> filterBySemanticRelevance(List<Document> results, String query, double threshold) {
        ...
        return results.stream()
                .filter(doc -> calculateRelevanceScore(doc, query) >= threshold)
                .toList();
    }
    ...
    private double calculateRelevanceScore(Document document, String query) {
        ...
        long matches = textFields.stream()
                .mapToLong(text -> {
                    String textLower = text.toLowerCase();
                    return queryLower.split("\\s+").length > 0 ? 
                           Arrays.stream(queryLower.split("\\s+"))
                                 .mapToLong(word -> textLower.contains(word) ? 1 : 0)
                                 .sum() : 0;
                })
                .sum();
